...
export default function App() {
  ...
  // Retrospective history rows from backend
  ...
  const loadData = useCallback(async () => {
    ...
    // 4) Closes (use locals so we can align dates synchronously)
    let datesLocal = [];
    let closesLocal = [];
    try {
    ...
    // 5) Retrospective history for past days (what models would have predicted)
    try {
  ...
  // ---------- Derived data for "Actual vs. Predicted" ----------
  const horizon = results?.[0]?.predictions?.length || 0;
  const actualTail = 60; // show recent 60 trading days
  const actualData = closes.slice(-actualTail);
  const actualLabels = closeDates.slice(-actualTail);

  // future labels (+1..+h) based on last known date
  const lastLabelDate = actualLabels.length
    ? new Date(actualLabels[actualLabels.length - 1])
    : null;

  const futureLabels = Array.from({ length: horizon }, (_, i) => {
    if (!lastLabelDate) return `+${i + 1}d`;
    const d = addBusinessDays(lastLabelDate, i + 1);
    return d.toISOString().slice(0, 10);
  });

  const labelsAll = [...actualLabels, ...futureLabels];

  // Build datasets: Actual, dashed backtest per model (from /predict_history), solid current forecast per model
  const colorPalette = ["#4e79a7", "#f28e2b", "#e15759", "#76b7b2", "#59a14f", "#edc949"];

  const avpDatasets = useMemo(() => {
    if (!labelsAll.length) return [];
    const actualSeries = [
      ...actualData,
    ...
    results.forEach((r, idx) => {
      ...
      // dashed backtest predictions (past dates only) from /predict_history
      const backtestSeries = labelsAll.map((lab) => {
      ...
      // solid current forecast starting from the latest actual
      const start = actualData[actualData.length - 1] ?? null;
      const currentSeries = [
    ...
    return ds;
  }, [results, labelsAll.join("|"), actualData, horizon, historyByDate]);

  const avpChartData = { labels: labelsAll, datasets: avpDatasets };
  ...
  const pastLabels = actualLabels.slice(-pastDaysToShow); // last N actual days
    ...
    <div className="app-root">
      ...
      <main className="container grid-2col">
        ...
        <section>
            ...
            <div className={`card ${blinkClass}`} style={{ minWidth: 300, flex: "0 1 320px" }}>
              ...
              {quoteErr ? (
                ...
                <>
                  ...
                  {/* mini interactive chart (clipped inside the card) */}
          ...
          {/* --- Actual vs Predicted (chart + table, with past backtest data) --- */}
...
/** Interactive SVG line chart with hover scrub, drag-pan, wheel-zoom + date labels (mini & modal) */
